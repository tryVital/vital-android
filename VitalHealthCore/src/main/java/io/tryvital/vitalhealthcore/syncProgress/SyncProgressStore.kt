...
class SyncProgressStore private constructor(
        ...
        ProcessLifecycleOwner.get().lifecycle.addObserver(
            object : LifecycleEventObserver {
                override fun onStateChanged(source: LifecycleOwner, event: Lifecycle.Event) {
                    latestProcessState.set(source.lifecycle.currentState)
    ...
    fun recordSync(
        ...
        mutate(listOf(id.resource.backfillType)) { res ->
            ...
            val appendsToLatest = latest?.start == id.start ||
                (status == SyncProgress.SyncStatus.deprioritized && latest?.lastStatus == SyncProgress.SyncStatus.deprioritized)
            ...
            if (appendsToLatest && latest != null) {
                ...
                when (status) {
                    ...
                    SyncProgress.SyncStatus.deprioritized -> latest.pruneDeprioritizedStatus(afterFirst = 10)
                    else -> Unit
                }
            } else {
                ...
                res.syncs += SyncProgress.Sync(
                    start = id.start,
                    statuses = mutableListOf(SyncProgress.Event(id.start, status, errorDetails = errorDetails)),
                    tags = augmentedTags,
                    dataCount = dataCount,
                )
            }
    ...
    fun recordSystem(resources: Collection<RemappedVitalResource>, eventType: SyncProgress.SystemEventType) {
    ...
    private inline fun mutate(backfillTypes: Collection<BackfillType>, action: (SyncProgress.Resource) -> Unit) {
        lock.withLock {
    ...
    private fun insertAppStateTags(tags: MutableSet<SyncProgress.SyncContextTag>) {
        when (latestProcessState.get()) {
            null -> tags += SyncProgress.SyncContextTag.appLaunching
            Lifecycle.State.INITIALIZED, Lifecycle.State.CREATED -> tags += SyncProgress.SyncContextTag.background
            Lifecycle.State.STARTED, Lifecycle.State.RESUMED -> tags += SyncProgress.SyncContextTag.foreground
