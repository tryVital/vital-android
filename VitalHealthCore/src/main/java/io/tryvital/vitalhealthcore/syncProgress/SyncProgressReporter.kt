...
class SyncProgressReporter(
    ...
    fun syncingResources(): Set<VitalResource> = lock.withLock { active.map { it.resource }.toSet() }
    ...
    private fun captureDeviceInfo(context: Context): SyncProgressReport.DeviceInfo {
        ...
        val packageInfo = appContext.packageManager.getPackageInfo(context.applicationContext.packageName, 0)
